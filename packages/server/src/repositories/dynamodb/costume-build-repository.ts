    ...
    try {
      await dynamodb.send(
        new UpdateCommand({
          ...
          UpdateExpression:
            "SET #name = :name, faceId = :faceId, upperId = :upperId, lowerId = :lowerId, shoesId = :shoesId, isDefault = :isDefault, version = :nextVersion",
          ConditionExpression: "version = :currentVersion",
          ExpressionAttributeNames: { "#name": "name" },
          ExpressionAttributeValues: {
            ":name": build.name,
            ":faceId": build.faceId ?? null,
            ":upperId": build.upperId ?? null,
            ":lowerId": build.lowerId ?? null,
            ":shoesId": build.shoesId ?? null,
